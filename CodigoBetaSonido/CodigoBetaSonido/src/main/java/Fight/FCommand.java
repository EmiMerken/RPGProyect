...
public class FCommand extends JPanel implements KeyListener {
    ...
    private void confirm(){
        try {
            if (buttons == null) {
                throw new ExcepcionBoton("Lista de botones no inicializada");
            }
            if (buttonsIndex < 0 || buttonsIndex >= buttons.size()) {
                throw new ExcepcionBoton("Índice de botón fuera de rango: " + buttonsIndex);
            }
            buttons.get(buttonsIndex).ejecuteAction();
        } catch (ExcepcionAccion | ExcepcionBoton e) {
            System.err.printf("Error en FCommand.confirm(): %s%n", e.getMessage());
